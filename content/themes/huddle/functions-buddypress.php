	...
	if ( !groups_is_user_member( $bp->loggedin_user->id, $group->id ) ) {
		...
		if ( 'public' == $group->status ) {
			...
			if ( !groups_join_group( $group->id ) ) {
				...
				echo '<a id="group-' . esc_attr( $group->id ) . '" class="leave-group btn-gray" rel="leave" title="' . __( 'Leave Group', 'huddle' ) . '" href="' . wp_nonce_url( bp_get_group_permalink( $group ) . 'leave-group', 'groups_leave_group' ) . '">' . __( 'Leave Group', 'huddle' ) . '</a>';
			}
			...
			if ( !groups_send_membership_request( $bp->loggedin_user->id, $group->id ) ) {
				...
				echo '<a id="group-' . esc_attr( $group->id ) . '" class="membership-requested btn-gray" rel="membership-requested" title="' . __( 'Membership Requested', 'huddle' ) . '" href="' . bp_get_group_permalink( $group ) . '">' . __( 'Membership Requested', 'huddle' ) . '</a>';
			}
		}
		...
		if ( !groups_leave_group( $group->id ) ) {
			...
			if ( 'public' == $group->status ) {
				echo '<a id="group-' . esc_attr( $group->id ) . '" class="join-group btn-gray" rel="join" title="' . __( 'Join Group', 'huddle' ) . '" href="' . wp_nonce_url( bp_get_group_permalink( $group ) . 'join', 'groups_join_group' ) . '">' . __( 'Join Group', 'huddle' ) . '</a>';
			} else if ( 'private' == $group->status ) {
				echo '<a id="group-' . esc_attr( $group->id ) . '" class="request-membership btn-gray" rel="join" title="' . __( 'Request Membership', 'huddle' ) . '" href="' . wp_nonce_url( bp_get_group_permalink( $group ) . 'request-membership', 'groups_send_membership_request' ) . '">' . __( 'Request Membership', 'huddle' ) . '</a>';
			}
		}
	}
}
